...
from opentalking.providers.stt.factory import normalize_stt_provider, stt_enabled_providers, stt_provider_config
...
def _enabled_provider_csv(current: list[str], provider: str) -> str:
    out = [item for item in current if item]
...
def _current_payload(settings: Any | None = None) -> dict[str, Any]:
    ...
    try:
        stt_provider = normalize_stt_provider(stt_provider, default="dashscope") or "dashscope"
    except ValueError:
...
def _build_updates(payload: RuntimeConfigPayload) -> dict[str, str]:
    ...
    tts_provider = ""
    if raw_tts_provider := _strip(payload.tts_provider):
        try:
            tts_provider = normalize_tts_provider(raw_tts_provider, default=None) or ""
    ...
    stt_provider = ""
    if raw_stt_provider := _strip(payload.stt_provider):
        try:
            stt_provider = normalize_stt_provider(raw_stt_provider, default=None) or ""
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        if stt_provider:
    ...
    if value := _strip(payload.stt_model):
        ...
        if not stt_provider or stt_provider == "dashscope":
