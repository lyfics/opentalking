...
def test_runtime_config_apply_persists_and_refreshes_settings(monkeypatch, tmp_path) -> None:
    ...
    assert os.environ["OPENTALKING_TTS_ENABLED_PROVIDERS"] == "edge,dashscope"
    assert os.environ["OPENTALKING_STT_ENABLED_PROVIDERS"] == "dashscope"
...
def test_runtime_config_accepts_supported_non_image_provider(monkeypatch, tmp_path) -> None:
    with _client(monkeypatch, tmp_path) as client:
        response = client.post(
            "/runtime-config/apply",
            json={
    ...
    assert response.status_code == 200
    assert response.json()["tts"]["provider"] == "indextts"
    assert response.json()["tts"]["enabled_providers"] == ["edge", "indextts"]
    text = (tmp_path / ".env").read_text(encoding="utf-8")
    assert "OPENTALKING_TTS_DEFAULT_PROVIDER=indextts" in text
    assert "OPENTALKING_TTS_ENABLED_PROVIDERS=edge,indextts" in text


def test_runtime_config_rejects_unknown_provider(monkeypatch, tmp_path) -> None:
    with _client(monkeypatch, tmp_path) as client:
        response = client.post(
            "/runtime-config/apply",
            json={
                "tts_provider": "not-a-provider",
                "stt_provider": "dashscope",
            },
        )
